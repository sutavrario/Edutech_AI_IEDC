...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.role === 'admin') {
        throw new Error("Please use the admin login portal (/admin-login).");
      }
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="pt-4 flex flex-col gap-4">
              ...
              <p className="text-center text-xs text-gray-400">
                Admin?{" "}
                <Link href="/admin-login" className="font-medium text-gray-500 hover:text-primary hover:underline transition-colors">
                  Login here
                </Link>
              </p>
            </div>
